...
export function isUserDisplaySwatchDisabled(hex, { primary, secondary, excludeUserId, adultMembers }) {
...
export function pickFirstAvailableUserDisplayColor({ primary, secondary, excludeUserId, adultMembers }) {
...
export function calendarEventAccentColor(ev) {
  if (!ev) return 'var(--primary)';
  return ev.color || ev.child_color || ev.creator_color || 'var(--primary)';
}
