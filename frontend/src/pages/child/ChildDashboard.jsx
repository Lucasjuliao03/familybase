    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-4 mb-24">
        <div className="stat-card" style={{boxShadow: '0 8px 20px rgba(108,92,231,0.1)'}}>
          ...
          <div className="stat-info"><h3>{child.points}</h3><p>{t('points')}</p></div>
        </div>
        <div className="stat-card" style={{boxShadow: '0 8px 20px rgba(253,203,110,0.2)'}}>
          <div className="stat-icon" style={{background:'rgba(253,203,110,0.15)',color:'#E67E22', fontSize:'1.5rem'}}>🪙</div>
          <div className="stat-info"><h3>{child.coins}</h3><p>Moedas</p></div>
        </div>
        <div className="stat-card" style={{boxShadow: '0 8px 20px rgba(225,112,85,0.1)'}}>
          ...
          <div className="stat-info"><h3>{child.streak_current}</h3><p>{t('streak')}</p></div>
        </div>
        <div className="stat-card" style={{boxShadow: '0 8px 20px rgba(0,184,148,0.1)'}}>
          ...
          <div className="stat-info"><h3>{profile?.stats?.medalsEarned || 0}</h3><p>{t('medals')}</p></div>
        </div>
      </div>
        ...
        <div className="card">
          ...
          <div className="grid grid-3" style={{gap:12}}>
            ...
            {(profile?.medals?.length || 0) === 0 && (
              <div className="empty-state" style={{gridColumn:'1/-1'}}><div className="empty-icon" style={{fontSize:'2rem'}}>🏅</div><h3 style={{fontSize:'0.9rem'}}>Ganhe pontos para liberar medalhas!</h3></div>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="card mt-24">
          ...
          <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(300px, 1fr))', gap: 16}}>
            {profile.recentHistory.slice(0, 6).map(h => (
              <div key={h.id} className="flex-between" style={{padding:'10px 12px', background:'var(--bg)', borderRadius: 8, border:'1px solid var(--border)'}}>
                <span style={{fontSize:'0.88rem', fontWeight:500}}>{h.event}</span>
                {h.points > 0 && <span className="badge badge-success" style={{borderRadius: 6}}>+{h.points} ⭐</span>}
              </div>
            ))}
          </div>
        </div>
