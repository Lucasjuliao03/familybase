import { shouldAutoRejectOccurrence } from '../shared/lib/taskOccurrenceClosure';
...
export function taskStatusLabel(occ: any): string {
...
export function taskStatusBadge(occ: any): string {
  const s = occ?.status || 'pending';
...
export function taskStatusTheme(occ: any) {
  const s = occ?.status || 'pending';
...
export function taskIcon(title = '', type = ''): string {
...
export function sortTasksForDisplay(list: any[]): any[] {
  const rank = (o: any) => {
...
export function filterOccurrences(occurrences: any[], filter: string): any[] {
...
export function countDelayed(occurrences: any[]): number {
  return occurrences.filter((o) => o.isDelayed || o.status === 'delayed').length;
}

export function canCompleteTask(occ: any, now: Date = new Date()): boolean {
