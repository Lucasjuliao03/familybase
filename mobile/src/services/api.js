...
} from '../shared/lib/taskOccurrenceClosure';
import {
  getProxyChildId,
  getProxyActorId,
  isChildProxyActive,
...
async function resolveAuthorizedChildUuidForWrites(body, familyId, authUserId) {
  ...
  if (role === 'parent' || role === 'relative' || role === 'master') {
    ...
    // Modo filho: sem child_id explícito, escreve no perfil do filho ativo.
    if (isChildProxyActive()) {
      const proxyChild = normalizeAuthChildUuid(getProxyChildId());
      if (proxyChild) {
        const { data: proxyOk } = await supabase
          .from('children')
          .select('id')
          .eq('family_id', familyId)
          .eq('id', proxyChild)
          .maybeSingle();
        if (proxyOk?.id) return proxyChild;
      }
    }
...
async function ensureOpenAllowanceCycleRow(supabase, familyId, childId) {
  ...
  const opening = settings?.allow_accumulation && prevRow?.final_amount != null && prevRow?.status !== 'paid' ? Number(prevRow.final_amount) : 0;
...
async function closeExpiredTaskOccurrencesForFamily(supabase, familyId, now = new Date()) {
  ...
  for (const occ of openOccs) {
    ...
    try {
      ...
      if (__DEV__) console.warn('[tasks] auto-reject:', e.message);
    }
  }
}
...
async function ensureRecurringOccurrencesForRange(supabase, familyId, fromYmd, toYmd) {
  ...
  for (let i = 0; i < rows.length; i += chunkSize) {
    ...
    if (upErr && __DEV__) console.warn('[tasks] ensure recurring range:', upErr.message);
  }
}
...
async function normalizeAvatarUpload(file) {
  if (!file || typeof file !== 'object') return null;
  if (file.uri) {
    const response = await fetch(file.uri);
    const blob = await response.blob();
    const type = file.type || blob.type || 'image/jpeg';
    const ext = (file.name && String(file.name).split('.').pop()) || type.split('/').pop() || 'jpg';
    const name = file.name || `avatar.${ext}`;
    return { payload: blob, name, type, size: blob.size };
  }
...
async function uploadAvatarToStorage(familyId, filePath, file) {
  const normalized = await normalizeAvatarUpload(file);
  if (!normalized) return null;
  let rel = null;
  for (const bucket of ['avatars', 'uploads']) {
    const { error: upErr } = await supabase.storage
      .from(bucket)
      .upload(filePath, normalized.payload, { upsert: true, contentType: normalized.type || undefined });
...
const api = {
  ...
  async get(url, config = {}) {
    ...
    if (path.startsWith('/tasks/occurrences')) {
      ...
      } else if (!occChildFilter && isChildProxyActive()) {
        // Modo filho: pai a atuar como o filho → escopo automático no filho ativo.
        occChildFilter = normalizeAuthChildUuid(getProxyChildId());
      }
    ...
    if (path.startsWith('/tasks')) {
      ...
      if (roleTk === 'child' && uidTk) {
        ...
        let scopedChildTpl = filterChildTpl;
        if (!scopedChildTpl && isChildProxyActive()) {
          scopedChildTpl = normalizeAuthChildUuid(getProxyChildId());
        }
        if (scopedChildTpl) tq = tq.eq('child_id', scopedChildTpl);
    ...
    if (path === '/allowance/estimated-balance') {
      ...
      if (!cycEb) {
        const { data: prevEb } = await supabase
          ...
          .maybeSingle();
        const openingEb =
          settingsEb.allow_accumulation && prevEb?.final_amount != null && prevEb?.status !== 'paid' ? Number(prevEb.final_amount) : 0;
  ...
  async post(url, body, config = {}) {
    ...
    if (path.startsWith('/grades')) {
      ...
      if (childIdAward) {
        // Se tirou nota máxima, dá 50 XP de bônus e registra no histórico
        const isMaxGrade = data.score != null && data.max_score != null && Number(data.score) === Number(data.max_score);
        if (isMaxGrade) {
          try {
            const { data: chRow } = await supabase.from('children').select('points, xp, level, xp_next_level').eq('id', childIdAward).single();
            if (chRow) {
              const bonusXp = 50;
              const newXp = (chRow.xp || 0) + bonusXp;
              let newLevel = chRow.level || 1;
              let newXpNext = chRow.xp_next_level || 100;
              if (newXp >= newXpNext) {
                newLevel++;
                newXpNext = Math.round(newXpNext * 1.5);
              }
              await supabase.from('children').update({
                points: (chRow.points || 0) + bonusXp,
                xp: newXp,
                level: newLevel,
              ...
              await supabase.from('history').insert({
                id: uuidv4(),
                family_id: familyId,
                child_id: childIdAward,
                event: `Nota máxima em ${data.subject || 'Matéria'} (+50 XP)! 📚`,
                points: bonusXp,
                created_at: new Date().toISOString(),
              });
            }
          } catch (eXp) {
            console.warn('[grades] erro ao dar bônus de nota máxima:', eXp);
          }
        }
    ...
    // ─── Fechamento do dia: marcar pendentes como não concluídas + descontos ──
    if (path === '/tasks/occurrences/close-day') {
      const dateStr = body?.date || toYMDLocal(new Date());
      // Buscar todas as ocorrências abertas da família nesta data
      const { data: openOccs, error: openErr } = await supabase
        .from('task_occurrences')
        .select('*, tasks(*, task_allowance_rules(*))')
        .eq('family_id', familyId)
      ...
      for (const occ of (openOccs || [])) {
        const task = occ.tasks;
        // autoRejectTaskOccurrence já: marca rejected + aplica desconto de mesada
        const result = await autoRejectTaskOccurrence(supabase, {
          familyId,
          userId,
          occ,
          task,
          reason: 'Não concluída no prazo do dia',
        });
        if (!result.skipped) closed.push(occ.id);
      }

      return { data: { closed: closed.length, date: dateStr, ids: closed } };
    }
    ...
    if (path === '/families/children') {
      ...
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          avatar_preset: body.avatar_preset || 'menino1',
    ...
    if (path === '/families/relatives') {
      ...
      if (Array.isArray(body.linked_child_ids) && body.linked_child_ids.length) {
        ...
        if (relChildErr && __DEV__) console.warn('[families] relative_children:', relChildErr.message);
      }
    ...
    if (path.startsWith('/allowance/cycles/') && path.endsWith('/close')) {
      ...
      const action = body?.action || 'pay'; // 'pay' ou 'rollover'
      const keepPreviousBase = body?.keep_previous_base === true;

      // 1. Atualizar o ciclo atual
      const { error: closeErr } = await supabase
        .from('allowance_cycles')
        .update({
          status: action === 'pay' ? 'paid' : 'closed',
          final_amount: finalAmt,
          closed_at: nowStr,
          paid_at: action === 'pay' ? nowStr : null,
          notes: action === 'pay' ? 'Ciclo encerrado e pago' : 'Ciclo encerrado e transferido',
        })
        .eq('id', cycleId)
        .eq('family_id', familyId);
      if (closeErr) throw new Error(closeErr.message || 'Erro ao fechar ciclo.');

      // Inserir log de transação
      if (action === 'pay') {
        // Zera o saldo gerando um débito compensatório no ciclo atual
        await supabase.from('allowance_transactions').insert({
          id: uuidv4(),
          family_id: familyId,
          ...
          type: 'debit',
          origin: 'payment',
          description: 'Pagamento e encerramento de mesada',
          amount: finalAmt,
          status: 'paid',
          approved_by: userId,
          balance_after: 0,
        ...
        // Apagar as configurações de mesada para voltar ao estado "Sem Mesada esperando configurar"
        await supabase
          .from('allowance_settings')
          .delete()
        ...
        // Ação: rollover (transferir/adicionar valor para o próximo mês)
        // Atualiza a data de corte nas configurações de mesada
        await supabase
          .from('allowance_settings')
          .update({ last_cycle_closed_at: nowStr })
          .eq('child_id', cycle.child_id)
          .eq('family_id', familyId);

        // Criar o ciclo seguinte automaticamente com saldo acumulado
        const nextMonth = cycle.month === 12 ? 1 : cycle.month + 1;
        const nextYear = cycle.month === 12 ? cycle.year + 1 : cycle.year;

        const base = keepPreviousBase ? (cycle.base_amount ?? settings?.base_amount ?? 0) : (settings?.base_amount ?? 0);
        const opening = finalAmt;

        // Se já existir ciclo aberto no próximo período, remover ou atualizar
        await supabase
          .from('allowance_cycles')
          .delete()
          .eq('child_id', cycle.child_id)
        ...
        if (insertedNext) {
          ...
          if (opening !== 0) {
            transRows.push({
              ...
              type: opening > 0 ? 'credit' : 'debit',
              origin: 'manual',
              description: 'Saldo transferido do ciclo anterior',
              amount: Math.abs(opening),
              status: 'approved',
              approved_by: userId,
              balance_after: 0
      ...
      // Registro de histórico/auditoria de ações de mesada
      try {
        await supabase.from('audit_logs').insert({
          family_id: familyId,
          user_id: userId,
          action: `allowance_cycle_${action}`,
          details: {
            child_id: cycle.child_id,
            cycle_id: cycleId,
            final_amount: finalAmt,
        ...
        console.warn('Failed to insert audit log for cycle closing:', logErr);
      }

      return { data: { ok: true, final_amount: finalAmt, action } };
    }
  ...
  async put(url, body, config = {}) {
    ...
    if (path.startsWith('/auth/avatar')) {
      ...
      let targetChildId = childMatch ? childMatch[1] : null;
      if (!targetChildId) {
        const role = await getUserRole();
        if (role === 'child' && userId) {
          targetChildId = await resolveViewerChildScope(familyId, userId);
        }
      }
      ...
      if (body instanceof FormData) {
        ...
        const upload = file ? await normalizeAvatarUpload(file) : null;
        if (upload) {
          const ext = (upload.name && String(upload.name).split('.').pop()) || 'jpg';
          const filePath = targetChildId
            ? `${familyId}/child-${targetChildId}-${Date.now()}.${ext}`
            : `${familyId}/user-${userId}-${Date.now()}.${ext}`;
          const rel = await uploadAvatarToStorage(familyId, filePath, file);

          if (targetChildId) {
            await supabase.from('children').update({ avatar_url: rel, avatar_preset: null }).eq('id', targetChildId).eq('family_id', familyId);
          } else {
        ...
        if (preset) {
          if (targetChildId) {
            await supabase.from('children').update({ avatar_preset: preset, avatar_url: null }).eq('id', targetChildId).eq('family_id', familyId);
          } else {
        ...
        if (targetChildId) {
          await supabase.from('children').update({ avatar_preset: body.avatar_preset, avatar_url: null }).eq('id', targetChildId).eq('family_id', familyId);
        } else {
    ...
    if (relativePutMatch) {
      ...
      if (body.relationship != null) {
        ...
        if (famMemErr && __DEV__) console.warn('[families] family_members:', famMemErr.message);
      }
      if (Array.isArray(body.linked_child_ids)) {
        ...
        if (rows.length) {
          ...
          if (relRowsErr && __DEV__) console.warn('[families] relative_children:', relRowsErr.message);
        }
      }
      return { data };
    }
    ...
    if (path === '/families/logo') {
      ...
      const upload = await normalizeAvatarUpload(file);
      if (!upload) throw new Error('Ficheiro de logo inválido');
      const ext = (upload.name && String(upload.name).split('.').pop()) || 'png';
      const filePath = `${familyId}/logo-${Date.now()}.${ext}`;
      const { error: upErr } = await supabase.storage.from('family-images').upload(filePath, upload.payload, { upsert: true, contentType: upload.type || undefined });
    ...
    if (completeMatch) {
      ...
      const proxyActive = isChildProxyActive();
      const { error } = await supabase
        .from('task_occurrences')
        .update({
          status: newStatus,
      ...
      // Auditoria: regista quem realmente concluiu (pai) quando em modo filho.
      // Update separado e best-effort para não quebrar caso a migração de
      // colunas (performed_by/acted_via_proxy) ainda não tenha sido aplicada.
      if (proxyActive) {
        try {
          await supabase
            .from('task_occurrences')
            .update({ performed_by: getProxyActorId() || userId, acted_via_proxy: true })
            .eq('id', occId);
        } catch { /* colunas de auditoria opcionais */ }
      }
    ...
    if (path.match(/^\/allowance\/redemptions\/[^/]+\/approve$/)) {
      ...
      if (approved && wasPending && updatedRed?.child_id) {
        ...
        if (cost > 0) {
          ...
          if (txErr && __DEV__) console.warn('[redemptions] lançamento mesada:', txErr.message);
        }
  ...
  async delete(url, config = {}) {
    ...
    if (path.startsWith('/allowance/rewards/')) {
      const rid = parts[2];
      await supabase.from('rewards').delete().eq('id', rid).eq('family_id', familyId);
      return { data: { success: true } };
    }
...
// ─── Auditoria do modo filho ────────────────────────────────────────────────
// Envolve as mutações (PUT/POST/DELETE) para que, quando o pai estiver a atuar
// como um filho, TODA alteração fique registada (performed_by = pai). A
// auditoria é best-effort e nunca quebra a operação principal.
const __wrapMutation = (fn, method) => async function wrapped(url, ...rest) {
  const result = await fn.apply(api, [url, ...rest]);
  try {
    if (isChildProxyActive()) {
      const body = method === 'delete' ? undefined : rest[0];
      await auditProxyMutation(method, url, body);
    }
  } catch { /* auditoria best-effort */ }
  return result;
};

api.put = __wrapMutation(api.put, 'put');
api.post = __wrapMutation(api.post, 'post');
api.delete = __wrapMutation(api.delete, 'delete');
